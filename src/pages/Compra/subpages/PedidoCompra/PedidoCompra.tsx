...
function PedidoCompra() {
  ...
  const [itensBoxInEdition, setItensBoxInEdition] = useState<ItensProduto>({
    listItemProduto: [],
  });
  const handleCompraChange = (itemId: number, value: number) => {
    const newItensProduto: ItemProduto[] =
      itensBoxInEdition.listItemProduto.map((item) =>
        item.itemId !== itemId
          ? item
          : {
              ...item,
  ...
  const handleItemBoxSave = (itemProd: ItemProduto) => {
    if (itemProd.itemId === 0) {
      alert(
        "Por favor marque o produto para incluir dentro da lista de compra."
      );
    } else {
  ...
  const fetchSetores = async () => {
    try {
      const response = await fetch(COMPRA_ROUTES.SETOR(), {
        method: "GET",
        headers: new Headers({
  ...
  const fetchSecoes = async () => {
    try {
      const response = await fetch(COMPRA_ROUTES.SECAO(), {
        method: "GET",
        headers: new Headers({
          "bypass-tunnel-reminder": "69420",
          "ngrok-skip-browser-warning": "69420",
  ...
  const fetchItensProduto = async () => {
    try {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        const newItensBoxInEdition: ItemProduto[] =
          itensProduto.listItemProduto.filter(
            (item) =>
              item.itemId !== 0 &&
              !itensBoxInEdition.listItemProduto.some(
                (item2) => item2.itemId === item.itemId
              )
          );

        if (newItensBoxInEdition.length > 0) {
          setItensBoxInEdition({
            listItemProduto: [
              ...itensBoxInEdition.listItemProduto,
              ...newItensBoxInEdition,
            ],
          });
        }
      } else {
  ...
  const fetchDeleteItemProduto = async (itemProd: ItemProduto) => {
    try {
      ...
      const newItensBox = itensBoxInEdition.listItemProduto.filter(
        (item) => item.itemId !== itemProd.itemId
      );
        ...
        if (secao.setorId === setor.id) {
          ...
          categorias.listCategoria.map((categoria: Categoria) => {
            if (categoria.secaoId === secao.id) {
              if (!map[setor.nome][secao.nome][categoria.nome])
                map[setor.nome][secao.nome][categoria.nome] = [];
              itensProduto.listItemProduto.map((itemProduto: ItemProduto) => {
                if (
                  itemProduto.catId === categoria.id &&
                  itemProduto.secaoId === secao.id &&
                  itemProduto.setorId === setor.id
                ) {
