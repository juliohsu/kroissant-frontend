...
import { Box, Button, Divider, FormControl, InputLabel, MenuItem, Paper, Select, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField } from "@mui/material";

import { Categoria, Categorias, Fornecedores, Marcas, Produto, Produtos, Unidade, Unidades } from "interfaces/compraInterfaces";
...
function ProdutoBib() {
    ...
    const [produtoDisplayJson, setProdutoDisplayJson] = useState<Produtos>({ listProduto: [] });
    ...
    // categoria state
    const [categoria_json, setCategoriaJson] = useState<Categorias>({ listCategoria: [] });
    ...
    // unidade state
    const [unidade_json, setUnidadeJson] = useState<Unidades>({ listUnidade: [] });
    ...
    const handleProdutoAdd = async () => {
        if (!produtoNome || !selectCategoria || !selectMarca || !selectUnidade || !selectFornecedor) {
            alert('Dados incompleto! Por favor informe todos os dados para adicionar o produto.');
        } else {
            ...
            <Box>
                <Box>
                    ...
                    >
                        ...
                        <Paper >
                            <TableContainer sx={{ height: '50vh', border: '1px solid black', borderRadius: 2, boxShadow: 2, overflowY: 'auto' }}>
                                <Table stickyHeader>
                                    <TableHead>
                                        <TableRow>
                                            <TableCell sx={{ color: 'white', backgroundColor: 'black' }}>ID</TableCell>
                                            <TableCell align='center' sx={{ color: 'white', backgroundColor: 'black' }}>Editar</TableCell>
                                            <TableCell align='center' sx={{ color: 'white', backgroundColor: 'black' }}>Nome</TableCell>
                                            <TableCell align='center' sx={{ color: 'white', backgroundColor: 'black' }}>Categoria</TableCell>
                                            <TableCell align='center' sx={{ color: 'white', backgroundColor: 'black' }}>Marca</TableCell>
                                            <TableCell align='center' sx={{ color: 'white', backgroundColor: 'black' }}>Unidade</TableCell>
                                            <TableCell align='center' sx={{ color: 'white', backgroundColor: 'black' }}>Fornecedor</TableCell>
                                            <TableCell align='center' sx={{ color: 'white', backgroundColor: 'black' }}>Excluir</TableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {
                                            produtoDisplayJson.listProduto.map((produto: Produto) => (
                                                <TableRow key={produto.id} sx={{ '&:last-child td, &:last-child th': { border: 0 }}}>
                                                    ...
                                                    <TableCell align='center'>
                                                        <ProdutoBibEdit id={produto.id} onCleanTextField={() => setProdutoDisplayTextField('')}/>
                                                    </TableCell>
                                                    ...
                                                    <TableCell align='center'>{produto.unidadeNome}</TableCell>
                                                    <TableCell align='center'>{produto.fornNome}</TableCell>
                                                    <TableCell align='center'>
                                                        <ProdutoBibDelete id={produto.id} onCleanTextField={() => setProdutoDisplayTextField('')}/>
                                                    </TableCell>
                                                </TableRow>
                                            ))
                                        }
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <Box sx={{ padding: 1 }}>
                        <FormControl fullWidth sx={{ marginBottom: 1.5 }} required>
                            <TextField value={produtoNome} onChange={handleDigitProdutoNome} label='Nome Produto' />
                        </FormControl>
                        <FormControl fullWidth sx={{ marginBottom: 1.5 }}>
                            <InputLabel>Categoria</InputLabel>
                            <Select value={selectCategoria} onChange={handleSelectCategoria} label='Categoria'>
                                {
                        ...
                        <FormControl fullWidth sx={{ marginBottom: 1.5 }}>
                            ...
                            <Select value={selectMarca} onChange={handleSelectMarca} label='Marca'>
                                {
                        ...
                        <FormControl fullWidth sx={{ marginBottom: 1.5 }}>
                            ...
                            <Select value={selectUnidade} onChange={handleSelectUnidade} label='Unidade Medida'>
                                {
                        ...
                        <FormControl fullWidth>
                            ...
                            <Select value={selectFornecedor} onChange={handleSelectFornecedor} label='Fornecedor'>
                                {
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', marginTop: '1rem' }}>
                            <Button variant='contained' color='secondary' onClick={handleProdutoAdd}>Adicionar Produto</Button>
                        </Box>
                    </Box>
                </Box>
            </Box>
